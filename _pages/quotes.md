---
layout: archive
title: ""
permalink: /quotes/
author_profile: true
redirect_from:
  - /quotes
---

> Men will set the goals, formulate the hypotheses, determine the criteria, and perform the evaluations. Computing machines will do the routinizable work that must be done to prepare the way for insights and decisions in technical and scientific thinking.
<div style="text-align: right">
  <em>J.C.R Licklider</em>
</div>

> The really decisive considerations from the present point of view, in selecting an instruction set, are more of a practical nature: simplicity of the equipment demanded by the instruction set, and the clarity of its application to the actually important problems together with the speed of its handling of those problems.
<div style="text-align: right">
  <em>von Neumann</em>
</div>

> In a major matter, no details are small.
<div style="text-align: right">
  <em>French proverb</em>
</div>

> When you grow up you tend to get told the world is the way it is and your life is just to live your life inside the world. Try not to bash into the walls too much. Try to have a nice family life, have fun, save a little money. That’s a very limited life. Life can be much broader once you discover one simple fact, and that is — everything around you that you call life, was made up by people who were no smarter than you. And you can change it, you can influence it, you can build your own things that other people can use.
<div style="text-align: right">
  <em>Steve Jobs</em>
</div>

> Engineering school prepares you for big projects, and a lot of guys wind up as transformer designers. It's a terrible let-down, I think. They end up with some rote engineering job with some thoroughly known technology that's repetitive, where all you have to do is look up the answers in books. We had the best high-energy story to tell a college graduate. They'd all heard about VAX. Well, we were gonna build a thirty-two-bit machine less expensive, faster and so on. You can sign up a guy to do that any day of the year.
<div style="text-align: right">
  <em>Tom West - Tracy Kidder, <i>The Soul of a New Machine</i></em>
</div>

> If the recruit said, for instance, "I'm really interested in computer design", then Alsing would prod. The ideal interview would proceed in this fashion:
>
> "What interests you about that?"
>
> "I want to build one", says the recruit.
>
> "That's what I want to hear", thinks Alsing. "Now I want to find out if he means it".
<div style="text-align: right">
  <em>Tracy Kidder, <i>The Soul of a New Machine</i></em>
</div>